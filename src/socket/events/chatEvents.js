...
//////////////////////////////////////////////////////////////////////////////
const GroupChatRoom = require('../../models/user/interchange/chat/GroupChatRoomModel');
...
exports.authorization = (io, socket, data) => {
  ...
  const isAdmin = userInfo.userInfo.userId === 'admin';
  if (isAdmin) {
    ChatGroup
  ...
  async.waterfall([
    ...
    (group, publicRoom, callback) => { // ----- Step3. Prepare updating info -----
      try {
        ...
        // user reading info
        const newMessagesLength = 0;//newMessages.length;
...
exports.roomLoadMoreMessages = (io, socket, data) => {
  ...
  ChatRoom
    ...
    .exec((err, rooms) => {
      ...
      const startPos = tmpStartPos < (userInfo?.msgStartIndex || 0) ? (userInfo?.msgStartIndex || 0) : tmpStartPos;
      ...
      ChatRoom
        .update(
          ...
          async (err2, updated) => {
            ...
            const userIds = _.map(room.users, user => user.userId);
            // const senderInfo = await User.findById(userId, 'avatar term birthday devRoomNo userId status stack realName netStarted gameId gender group team ip ipMsgId netKeyId ').populate({ path: "group team", select: "title _id" })
            const roomGroup = await ChatGroup.findById(room.group, 'title')
            const updatedRoom = {
              ...room,
              roomGroup,
              users: updatedUsers,
...
exports.roomMute = async (io, socket, data) => {
  if (!chatUtils.checkAuthorized(io, socket)) return;
  ...
  if (isMuted)
    await ChatRoom.updateOne({ _id: roomId }, { $push: { mutedUsers: { userId: userId } } })
  else
    await ChatRoom.updateOne({ _id: roomId }, { $pull: { mutedUsers: { userId: userId } } })

  socket.emit(CONSTS.CHAT_CONSTS.S2C_CHAT_ROOM_SET_MUTE, { sender: userId, roomId, isMuted });
}

//////////////////////////////////////////////
exports.userEnter = async (io, socket, data) => {

  const user = data
  socket.join(user)
}

exports.groupEnter = async (io, socket, data) => {

  const groupId = data
  socket.join(groupId)
}

exports.groupRoomEnter = async (io, socket, data) => {

  const groupRoomId = data
  socket.join(groupRoomId)
}

exports.groupFriendRoomEnter = async (io, socket, data) => {

  const groupFriendRoomId = data
  socket.join(groupFriendRoomId)
}

exports.groupChannelRoomEnter = async (io, socket, data) => {

  const groupChannelRoomId = data
  socket.join(groupChannelRoomId)
}

exports.groupRoomMessageNew = async (io, socket, data) => {
  ...
  await GroupChatRoom.create(newMessage);
  ...
  try {
    ...
    io.of('/socket').to(roomId).emit('S2C_GET_ALL_GROUP_MESSAGE', {results: results})
    io.of('/socket').to('allGroups').emit('S2C_SEND_ALL_USER_MESSAGE', {results: results})
...
exports.groupFriendMessageNew = async (io, socket, data) => {
...
exports.addFriend = async (io, socket, data) => {
  ...
  if(friend?.length > 0) {
    ...
    try {
      ...
      io.of('/socket').to(groupId).emit('S2C_ADD_FRIEND', {friend: results})
    ...
    await UserFriendModel.create(newFriend1);
    await UserFriendModel.create(newFriend2);
